"""Cosmos3 video scheduler for single-rank and model-parallel replicas."""
...
class Cosmos3T2VScheduler(Scheduler):
    """Video and sound denoising over the replica's declared rank groups."""
    ...
    def step(
        ...
        # halves of the ``cfg`` group (each a full ``tp`` group). This rank computes
        ...
        if self.cfg_size > 1:
            ...
            with event_scope(scope):
                for timestep in self.unipc.timesteps:
                    ...
                    if with_sound:
                        s_pair = P.all_gather(s_local.unsqueeze(0), group="cfg", dim=0)
                        s_vel = combine_cfg(
                            s_pair[0], s_pair[1], request.guidance_scale
                        )
