...
# 前向：kernel catalog 决定 kernel

层不自己写 fp8 / cutlass / marlin 的分支。`LinearBase.forward` 把这次调用描述成结构化事实——来自 `spec.physical_signature` 的精确存储格式、dtype、以及 `M`/`N`/`K`——由 kernel selector 选出能力声明匹配的 catalog 行：
...
选择结果按层由 `CallSite` 记忆，键只覆盖 catalog 行和当前 policy 真正读取的事实（dtype、`N`/`K`、量化签名、执行模式），重复调用只花一次字典查找。哪种存储格式跑在哪里，由 `phyai/kernel/ops/gemm.py` 里的 catalog 行逐条声明：

| spec | physical signature |
| --- | --- |
| `Bf16Spec` | `format=bf16` |
| `Fp8Spec` | `format=fp8_e4m3`，`granularity=per_tensor / per_channel / block` |
| `Nvfp4Spec` | `format=nvfp4`，`layout=128x4 / linear` |
