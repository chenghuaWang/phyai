...
<ModelCard
  ...
  rows={{
    ...
    "Tokenizer 长度": "48",
    "运行入口": <code className="px-2 py-0.5 rounded bg-[#003399]/10 dark:bg-[#60A5FA]/15 text-[#003399] dark:text-[#60A5FA] text-xs font-mono">PI0Scheduler</code>,
...
`PI0Scheduler` 只实现这条单卡路径，不承担 tensor parallel、continuous batching 或 preemption。
...
| `PI0Scheduler` | 在单张 GPU 上编排一次完整的 inference request |
...
CUDA graph 能够复用，前提是请求形状保持固定。`PI0Scheduler` 构造单卡 Engine 时会固定 `max_batch_size`，graph capture 还会固定相机数量、图像尺寸和 tokenizer 长度。任何一项发生变化，都需要重建 Engine。Vision tower 则按真实 batch 中的样本逐个 replay，并不会把这些样本合成一次 vision batch。
