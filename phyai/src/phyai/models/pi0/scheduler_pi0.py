"""PI0 request scheduler for one complete model replica.
...
The PI0 rank plan is currently a singleton. Serving additional replicas is a
deployment concern outside this scheduler.
"""
...
class PI0Scheduler(Scheduler):
    """Run complete PI0 requests with a fixed maximum batch size."""
    ...
    def step(self, request: PI0Request) -> torch.Tensor:
        ...
        with event_scope("pi0.expert_loop"):
            if self.time_emb_table is None:
                raise RuntimeError(
                    "PI0Scheduler.step() called before setup(); "
