"""Cosmos3 policy scheduler for single-rank and model-parallel replicas."""
...
from phyai.models.cosmos3.parallel_cosmos3 import group_rank_size
from phyai.models.cosmos3.sampler_unipc import UniPCMultistepSampler
from phyai.models.cosmos3.requests_cosmos3_policy import (
    ACTION_MODES,
...
class Cosmos3PolicyScheduler(Scheduler):
    """Action and video denoising over the replica's declared rank groups."""
    ...
    def __init__(
        ...
        *,
        ...
        self.tp_rank, self.tp_size = group_rank_size("dense_tp")
        self.cfg_rank, self.cfg_size = group_rank_size("cfg")
        if self.cfg_size not in (1, 2):
            raise ValueError(
    ...
    def step(
        ...
        if request.mode not in ACTION_MODES:
            raise ValueError(
                f"mode must be one of {ACTION_MODES}, got {request.mode!r}."
            )

        if request.cond_video_latents is None and request.cond_video_pixels is not None:
            if self.vae_runner is None:
                raise RuntimeError(
                    "Cosmos3PolicyScheduler requires a VAE to encode "
        ...
        # of the ``cfg`` group; this rank computes one branch, an all-gather over
        ...
        if self.cfg_size > 1:
            ...
            with event_scope("cosmos3.policy_denoise_loop"):
                for timestep in uni_v.timesteps:
                    ...
                    v_pair = P.all_gather(v_local.unsqueeze(0), group="cfg", dim=0)
                    a_pair = P.all_gather(a_local.unsqueeze(0), group="cfg", dim=0)
                    v_vel = combine_cfg(v_pair[0], v_pair[1], request.guidance_scale)
                    a_vel = combine_cfg(a_pair[0], a_pair[1], request.guidance_scale)
                    a_vel[:, :, raw:] = 0.0
