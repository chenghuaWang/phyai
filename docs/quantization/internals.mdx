...
# The forward pass: the kernel catalog picks the kernel

Layers do not write fp8 / cutlass / marlin branches themselves. `LinearBase.forward` describes the call as structured facts — the exact storage format from `spec.physical_signature`, the dtypes, and `M`/`N`/`K` — and the kernel selector picks the catalog row whose declared capability matches:

```python
kernel = _resolve_linear_kernel(
    self, x, M=_M_of(x), N=self.out_features, K=self.in_features
)
y = kernel.execute(self, x, bias)
```

Selection is memoized per layer by a `CallSite` whose key covers exactly the facts the catalog rows and the active policy read (dtype, `N`/`K`, the quantization signature, execution mode), so repeated calls cost one dictionary lookup. Which storage format runs where is declared per catalog row in `phyai/kernel/ops/gemm.py`:

| spec | physical signature |
| --- | --- |
| `Bf16Spec` | `format=bf16` |
| `Fp8Spec` | `format=fp8_e4m3` with `granularity=per_tensor / per_channel / block` |
| `Nvfp4Spec` | `format=nvfp4` with `layout=128x4 / linear` |
